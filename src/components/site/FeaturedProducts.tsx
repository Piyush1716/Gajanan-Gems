...
import { getSocialProof } from "@/lib/offers";
        ...
        {!loading && !error && (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-5">
            {products.filter(p => p.bestseller).slice(0, 12).map((p) => {
              ...
              const proof = getSocialProof(p.id);
              const isBracelet = p.categoryName?.toLowerCase().includes("bracelet");
                ...
                >
                  <Link to="/product/$slug" params={{ slug: p.slug }} className="block">
                    <div className="relative aspect-square overflow-hidden bg-secondary">
                      ...
                      {/* Priority badge — replaces old plain tag */}
                      <ProductBadge product={p} variant="card" />

                      {/* Discount % badge (top-right, only when no other badge occupies top-left) */}
                      {discount > 0 && (
                        <span
                    ...
                    <div className="p-3 sm:p-4">
                      ...
                      <div className="mt-1.5 flex items-center gap-2 flex-wrap">
                        <span className="text-[10px] text-muted-foreground">🔥 {proof.viewed} viewed today</span>
                        <span className="text-[10px] text-muted-foreground">⚡ {proof.left} left</span>
                      </div>
                    </div>
                  </Link>

                  <div className="px-3 sm:px-4 pb-3 sm:pb-4 mt-auto flex flex-col gap-1.5">
                    <button
                    ...
                    {/* Qty discount hint — bracelet category only, informational */}
                    {isBracelet && (
                      <p
                        className="text-center text-[9px] sm:text-[10px] font-medium rounded-full py-1 px-2"
                        style={{ backgroundColor: "#FEF3C7", color: "#92400E" }}
                      >
                        Buy 2 → 10% OFF · Buy 3 → 15% OFF
                      </p>
                    )}
                  </div>
                </div>
